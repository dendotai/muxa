...
<Tabs defaultValue="basic">
  <Tab value="basic" label="Basic">
    ```bash # Run multiple commands muxa 'npm run dev' 'npm run test:watch' ```
  </Tab>
  <Tab value="workspace" label="Workspace">
    ```bash # Run scripts in specific packages muxa -s backend dev -s frontend start ```
  </Tab>
  <Tab value="mixed" label="Mixed">
    ```bash # Mix workspace scripts and regular commands muxa -s backend dev -c 'docker-compose up'
    db ```
  </Tab>
</Tabs>
...
Check out the [installation guide](/muxa/installation) to get started, or jump straight to [usage examples](/muxa/usage).
